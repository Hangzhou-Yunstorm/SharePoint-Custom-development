$(function () {
    ...
    var td = "<td id='td_title'><div class='splitters' style='border-bottom:1px dashed #ccc; height:20px; padding-bottom: 10px;'>" +
        "<h2 style='float:left;height:20px;line-height:20px;'>Subscription</h2>" +
        "<img  onclick='SubscribeDownload()' style='float:right;cursor:pointer;' src='/_layouts/15/Images/down1.png' alt='Download file'></div></td>";
...
function SubscribeDownload() {
    ...
    if (num == 1) {
        ...
        var url = ctx.HttpRoot + '/_layouts/15/SubscribeDownload.aspx?ItemID=' + id;
        window.open(url);
    }
    else if (num > 0) {
        var ids = '';
...
//批量下载
function SubscribeDownloadAll(ids) {
    ...
    $.ajax({
        type: "post",
        url: "/_layouts/15/SubscribeMulDownload.aspx/SubscribeDownLoadZIP",
