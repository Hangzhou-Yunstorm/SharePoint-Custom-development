...
function SubmitData() {
    ...
    if (containSpecial(groupName)) {
        alert("The user group name can not contain any of the following characters: | \ \" ' / [ ] : < > + = , ; ? * @ ");
        return;
    }
    ...
    $.ajax({
        type: "post",
        url: "AddUsers.aspx/AddGroup",
        data: "{ 'groupName':'" + groupName + "','userNames':'" + userNames + "'}",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        success: function (response, textStatus) {
            var msg = response.d;
