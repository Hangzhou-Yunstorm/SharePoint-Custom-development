...
{
    public partial class GetMoWaterMark : IHttpHandler
    {
        ...
        private byte[] GetPng(string waterWords)
        {
            ...
            Font crFont = new Font("Microsoft YaHei", 20, FontStyle.Italic);
            SizeF crSize = g.MeasureString(waterWords, crFont);
            int strX = (int)((float)width - crSize.Width) / 2 - 10;
            int strY = (int)((float)height - crSize.Height) / 2;

            g.TranslateTransform(0, strY); //设置旋转中心为文字中心
            g.RotateTransform((float)(-15)); //旋转
